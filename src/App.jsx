...
function App() {
  ...
  const [statsRefresh, setStatsRefresh] = useState(0); // Counter to force stats refresh
  ...
  const handleCreateLocation = async (name) => {
    if (!name.trim()) return;
    const { success, location } = await addLocation(currentPantryId, name);
    if (success && location) {
  ...
  const handleAddLocation = async (e) => {
    ...
    await handleCreateLocation(newLocationName);
    setNewLocationName('');
  ...
  const handleUpdate = async () => {
    await loadProducts();
    // Increment stats refresh counter to trigger re-render of WasteDashboard
  ...
  return (
    <div className="h-[100dvh] flex flex-row overflow-hidden text-gray-800 font-sans transition-colors duration-500 bg-gradient-to-br from-teal-50 via-orange-50 to-rose-50 dark:from-pantry-bg-dark dark:to-pantry-bg-secondary">
      ...
      {/* RIGHT COLUMN WRAPPER (Mobile: Full width, Desktop: Next to sidebar) */}
      <div className="flex-1 flex flex-col h-full relative w-full max-w-full">
        ...
        <header className="px-6 py-5 pt-[calc(1.25rem+env(safe-area-inset-top))] flex justify-between items-center z-30 shrink-0">
          <div>
            {/* Location Switcher Header - Visible only on Mobile now for the switcher part, but we might want the Title always? 
              Actually, let's hide the switcher trigger on desktop since it's in the Sidebar 
          */}
          ...
          {/* DESKTOP ADD BUTTON - Visible only on Desktop */}
          {activeTab === 'pantry' && (
            <button
              onClick={() => setIsFormOpen(true)}
              className="hidden md:flex items-center gap-2 px-5 py-3 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-2xl font-bold shadow-lg hover:scale-105 active:scale-95 transition-all"
            >
        ...
        <main className="flex-1 overflow-y-auto overflow-x-hidden px-5 pb-[calc(8rem+env(safe-area-inset-bottom))] pt-2 scroll-smooth no-scrollbar">
          <div className="max-w-md md:max-w-full mx-auto h-full">
            {activeTab === 'pantry' && (
              <ProductList
                products={
                  activeLocation
            ...
            {activeTab === 'stats' && (
              <div className="h-full">
                <WasteDashboard key={statsRefresh} />
              </div>
            )}
        ...
        {activeTab === 'pantry' && (
          <button
            onClick={() => setIsFormOpen(true)}
            className="md:hidden fixed bottom-32 right-6 w-16 h-16 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-[24px] shadow-2xl flex items-center justify-center hover:scale-105 active:scale-95 transition-all z-30 border border-white/20 dark:border-black/10"
          >
